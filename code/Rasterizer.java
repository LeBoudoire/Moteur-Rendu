...
public class Rasterizer {
	...
	*/
    ...
    public void rasterizeFace (Fragment v1, Fragment v2, Fragment v3) {
        ...
        for(float x = x_min;x<= x_max;x=x+0.1f) {
        	for(float y= y_min;y<= y_max;y=y+0.1f) {
        		try {
        			Vector proj = new Vector3(1,x,y);
					Vector bary = C.multiply(proj);
					if ((bary.get(0) >= 0) && (bary.get(1) >= 0) &&(bary.get(2) >=0)) {
						int x_new = (int) (bary.get(0)*v1.getX() + bary.get(1)*v2.getX() + bary.get(2)*v3.getX());
						int y_new = (int) (bary.get(0)*v1.getY() + bary.get(1)*v2.getY() + bary.get(2)*v3.getY());
						Fragment frag = new Fragment(x_new,y_new);
						for (int i = 0; i<9; i++) {
							double att_new = bary.get(0)*v1.getAttribute(i) + bary.get(1)*v2.getAttribute(i) + bary.get(2)*v3.getAttribute(i);
							frag.setAttribute(i, att_new);
						}
						shader.shade(frag);
					}
